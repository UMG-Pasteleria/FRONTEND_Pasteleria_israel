...
const PDFGenerator = ({ data }) => {
  const generatePDF = () => {
    ...
    const tableColumn = ['No.', 'Responsable', 'Producto', 'Cantidad', 'Metodo de pago', 'Emision', 'Entrega'];
    const tableRows = [];

    data.forEach((item) => {
      const rowData = [++contador, item.responsable, item.nombre_producto, item.cantidad, item.metd_pago,
                                    item.emision, item.entrega];
