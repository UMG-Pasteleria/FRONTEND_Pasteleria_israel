...
const PDFGenerator = ({ data }) => {
  const generatePDF = () => {
    ...
    const tableColumn = ['No.', 'Nombre', 'NIT', 'Teléfono', 'Direccion', 'Tipo'];
    const tableRows = [];

    data.forEach((item) => {
      const rowData = [++contador, item.nombre_cl, item.nit_cl, item.telefono_cl, item.direccion_cl, item.tipo_cliente];
  ...
  return (
    <div>
      ...
      <span  onClick={generatePDF}  className="material-symbols-outlined">print</span>
        </button>
    </div>
...
PDFGenerator.propTypes = {
  data: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      nit_cl: PropTypes.string,
      telefono_cl: PropTypes.string,
