...
const ModalupCatPastel = ({
  ...
  const onChangeData = (e) => {
    setCatPasteloUP({ ...catpastelUP, [e.target.name]: e.target.value });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setDecPasteles(
        catpasteles.map((catpastel) =>
          catpastel.idcatp === catpastelUP.idcatp ? catpastelUP : catpastel
        )
      );
      ...
      if (response.status === 200) {
        swal.fire({
          title: "Categoria De Pastel Actualizado!",
          icon: "success",
...
export default ModalupCatPastel;
